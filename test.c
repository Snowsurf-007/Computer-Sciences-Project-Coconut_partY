...
void deplacement_attaquant(Case** carte, int i, int j){
    int type_attaquant = carte[i][j].type;

    if (type_attaquant != 8 && type_attaquant != 9 && type_attaquant != 10){
        return;
    }

    // Essayer d'aller en bas
    if (carte[i+1][j].type == 6 || carte[i+1][j].type == 7) {
        carte[i+1][j].type = type_attaquant;
        carte[i][j].type = 6;
        return;
    }

    // À droite
    if (carte[i][j+1].type == 6 || carte[i][j+1].type == 7) {
        carte[i][j+1].type = type_attaquant;
        carte[i][j].type = 6;
        return;
    }
    ...
    // Si aucun déplacement possible, on ne fait rien
}
...
int main (){
	...
	for (int j=0; j<taillecarte; j++) {
    	...
    	afficher_carte(carte, taillecarte);
    	deplacement_attaquant(carte, 0, colonneDebut);
    	afficher_carte(carte, taillecarte);
