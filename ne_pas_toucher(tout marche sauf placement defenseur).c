...
void placement_de_defenseur(Case** carte, int taillecarte, int* monnaie, Defenseur** liste_defenseur, int* nb_defenseur) {
    ...
    while (placer == 1) {
        ...
        if (choix_defenseur == 1) {
            nouv_def = constructeur_PinguPatrouilleur((Defenseur){0});
        } else if (choix_defenseur == 2) {
            nouv_def = constructeur_FloconPerceCiel((Defenseur){0});
        } else if (choix_defenseur == 3) {
            nouv_def = constructeur_GardePolaire((Defenseur){0});
        } else {
            exit(1);
        }

        if (*monnaie < nouv_def.prix) {
            printf("Monnaie insuffisante pour placer ce défenseur. Vous avez %d flocons, mais il en faut %d.\n", *monnaie, nouv_def.prix);
            return;
        }

        printf("Où souhaitez-vous placer le défenseur ? Choisissez une coordonnée x (ligne).\n");
        scanf("%d", &coord_x);
        while (coord_x >= taillecarte || coord_x < 0) {
            printf("La valeur entrée est incorrecte, veuillez réessayer.\n");
            printf("Choisissez une coordonnée x (ligne).\n");
            scanf("%d", &coord_x);
        }

        printf("Veuillez maintenant choisir la coordonnée y (colonne).\n");
        scanf("%d", &coord_y);
        while (coord_y >= taillecarte || coord_y < 0) {
            printf("La valeur entrée est incorrecte, veuillez réessayer.\n");
            printf("Choisissez une coordonnée y (colonne).\n");
            scanf("%d", &coord_y);
        }

        // Vérifie si la case est valide pour placer un défenseur
        if (carte[coord_x][coord_y].type != 0 && carte[coord_x][coord_y].type != 1 &&
            carte[coord_x][coord_y].type != 2 && carte[coord_x][coord_y].type != 3) {
            printf("Impossible de placer un défenseur sur cette case. Veuillez choisir une autre case.\n");
            continue;
        }

        // Déduit le coût du défenseur de la monnaie
        *monnaie -= nouv_def.prix;

        // Place le défenseur sur la carte
        carte[coord_x][coord_y].type = choix_defenseur + 10; // Défenseur placé sur la carte
        nouv_def.coordx = coord_x;
        nouv_def.coordy = coord_y;

        // Ajoute le défenseur à la liste des défenseurs
        *liste_defenseur = realloc(*liste_defenseur, (*nb_defenseur + 1) * sizeof(Defenseur));
        if (*liste_defenseur == NULL) {
            printf("Erreur d'allocation mémoire pour la liste des défenseurs.\n");
            exit(1);
        }
        (*liste_defenseur)[*nb_defenseur] = nouv_def;
        (*nb_defenseur)++;

        printf("Défenseur placé avec succès en (%d, %d) !\n", coord_x, coord_y);
        printf("Flocons restants : %d\n", *monnaie);

        // Demande si l'utilisateur souhaite placer un autre défenseur
...
void lancerpartie(Case*** carte, int* taillecarte, Defenseur** defenseurs, int* nbDefenseurs, EnnemiActif** ennemis, int* nbEnnemis, int* score, int* flocon, int* vague) {
	...
	for(*vague; *vague<16; (*vague)++){
		compteur=0;
		placement_de_defenseur(*carte, *taillecarte, flocon, &defenseur, nb_defenseur);
