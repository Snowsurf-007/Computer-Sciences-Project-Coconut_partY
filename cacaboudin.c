...
void attaquer_defenseurs(Case** carte, Defenseur* defenseurs, int* nbDefenseurs, EnnemiActif* ennemis, int* nbEnnemis, int* score) {
    for (int i = 0; i < *nbDefenseurs-1; i++) {
        ...
        for (int j = 0; j < *nbEnnemis; j++) {
            ...
            printf("  Distance calculée : %.2f\n", distance);
            printf("  portée du def : %.2f\n", def.portee);
...
int main() {
    ...
    int flocons=350;
    int vague=0;
    Defenseur* defenseurs=(Defenseur*)malloc(100 * sizeof(Defenseur)); // Pre-allocation d'espace pour 100 defenseursf
    if (defenseurs == NULL) {
